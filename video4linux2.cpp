...
VideoInit(State*state)
{
  ...
  sleep(3);
  assert(-1!=ioctl(state->fd,VIDIOC_S_FMT,&format));
  ...
  for(i=0;i<reqbuf.count;i++){
    ...
    buf.index=i;
    assert(-1!=ioctl(state->fd,VIDIOC_QUERYBUF,&buf));
    ...
    assert(state->buffers[i].start!=MAP_FAILED);
 
    assert(-1!=ioctl(state->fd,VIDIOC_QBUF,&buf));
  }
  state->buffers_count=reqbuf.count;
  int type=V4L2_BUF_TYPE_VIDEO_CAPTURE; 
  assert(-1!=ioctl(state->fd,VIDIOC_STREAMON,&type));
}
